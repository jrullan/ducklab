...
its ownership. `Produces` records the artifact's unique creator; `Modifies`
records a later writer and serializes it behind the previous writer without
claiming a second creation. Shared registries and integration files belong to an explicit
