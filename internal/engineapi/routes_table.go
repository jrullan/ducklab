...
func routeTable() []Route {
	return []Route{
		...
		{Method: "GET", Path: "/v1/projects/{id}/diagnostics", Auth: true,
			Response: service.ConfigDiagnostics{}, Summary: "Read-only remote and git tooling diagnostics", ClientMethod: "ConfigDiagnostics",
			handler: func(s *Server) http.HandlerFunc { return s.handleConfigDiagnostics }},
		...
		{Method: "GET", Path: "/v1/projects/{id}/app", Auth: true,
			Response:     service.AppStatus{},
			Summary:      "The application's run configuration and managed-process state",
			ClientMethod: "AppStatus",
			handler:      func(s *Server) http.HandlerFunc { return s.handleAppStatus }},
		{Method: "POST", Path: "/v1/projects/{id}/app/start", Auth: true,
			Response:     service.AppStatus{},
			Summary:      "Start the app via run.command as an engine-managed process",
			ClientMethod: "AppStart",
			handler:      func(s *Server) http.HandlerFunc { return s.handleAppStart }},
		{Method: "POST", Path: "/v1/projects/{id}/app/stop", Auth: true,
		...
		{Method: "GET", Path: "/v1/projects/{id}/status", Auth: true,
			Response: service.Status{}, Summary: "Project status, including ahead/behind publication state when divergent", ClientMethod: "ProjectStatus",
		...
		{Method: "GET", Path: "/v1/ducklings/scorecards", Auth: true,
		...
		{Method: "GET", Path: "/v1/defaults/engine", Auth: true,
			...
			handler: func(s *Server) http.HandlerFunc { return s.handleEngineDefaults }},
		{Method: "PUT", Path: "/v1/defaults/engine", Auth: true,
			Request: service.EngineDefaultsView{}, Summary: "Replace the engine concurrency cap", ClientMethod: "EngineDefaultsSet",
			handler: func(s *Server) http.HandlerFunc { return s.handleEngineDefaultsSet }},
		{Method: "GET", Path: "/v1/defaults/diagnostics", Auth: true,
			Response: service.DiagnosticDefaultsView{}, Summary: "Registered harness project available to cross-project consultant diagnostics", ClientMethod: "DiagnosticDefaults",
			handler: func(s *Server) http.HandlerFunc { return s.handleDiagnosticDefaults }},
		{Method: "PUT", Path: "/v1/defaults/diagnostics", Auth: true,
			Request: service.DiagnosticDefaultsView{}, Response: service.DiagnosticDefaultsView{}, Summary: "Select the registered harness project used by consultant diagnostics", ClientMethod: "DiagnosticDefaultsSet",
			handler: func(s *Server) http.HandlerFunc { return s.handleDiagnosticDefaultsSet }},
		...
		{Method: "GET", Path: "/v1/defaults/autopilot", Auth: true,
			Summary:      "Autopilot and autonomy defaults",
			ClientMethod: "AutopilotDefaults",
			handler:      func(s *Server) http.HandlerFunc { return s.handleAutopilotDefaults }},
		{Method: "PUT", Path: "/v1/defaults/autopilot", Auth: true,
		...
		{Method: "GET", Path: "/v1/runs/{id}/captures/{name}", Auth: true,
			Summary: "Attached render capture", ClientMethod: "RunCapture",
			handler: func(s *Server) http.HandlerFunc { return s.handleRunCapture }},
		...
		{Method: "POST", Path: "/v1/runs/{id}/findings/file", Auth: true,
			Response:     bugItems{},
			Summary:      "File the run's final reviewer findings as bug reports, with provenance",
			ClientMethod: "RunFileFindings",
		...
		{Method: "GET", Path: "/v1/projects/{id}/bugs/{bug}", Auth: true,
		...
		{Method: "POST", Path: "/v1/projects/{id}/tasks/{task}/land", Auth: true,
			Request: service.TaskLandRequest{}, Response: runlog.Run{},
			Summary: "Declare a task completed by a commit already reachable from the default branch", ClientMethod: "TaskLand",
			handler: func(s *Server) http.HandlerFunc { return s.handleTaskLand }},
		...
		{Method: "GET", Path: "/v1/projects/{id}/autonomy", Auth: true,
			Summary: "The project's declared autonomy", ClientMethod: "ProjectAutonomy",
			handler: func(s *Server) http.HandlerFunc { return s.handleProjectAutonomyGet }},
		{Method: "PUT", Path: "/v1/projects/{id}/autonomy", Auth: true,
			Summary: "Set the project's autonomy", ClientMethod: "ProjectAutonomySet",
			handler: func(s *Server) http.HandlerFunc { return s.handleProjectAutonomySet }},
		{Method: "GET", Path: "/v1/projects/{id}/autopilot", Auth: true,
			Summary: "Autopilot state for a project", ClientMethod: "AutopilotStatus",
			handler: func(s *Server) http.HandlerFunc { return s.handleAutopilotGet }},
		{Method: "POST", Path: "/v1/projects/{id}/autopilot", Auth: true,
			Summary: "Turn the autopilot on or off", ClientMethod: "AutopilotSet",
			handler: func(s *Server) http.HandlerFunc { return s.handleAutopilotSet }},
		...
		{Method: "POST", Path: "/v1/runs/{id}/chat/end", Auth: true,
			Response: runlog.Run{}, Summary: "End a chat as finished — a done consultation is not an abort",
			ClientMethod: "ChatEnd",
			handler:      func(s *Server) http.HandlerFunc { return s.handleChatEnd }},
		{Method: "POST", Path: "/v1/runs/{id}/chat", Auth: true,
			Request: chatSendRequest{}, Response: runlog.Run{},
			Summary:      "Send the next message in a paused chat",
			ClientMethod: "ChatSend",
			handler:      func(s *Server) http.HandlerFunc { return s.handleChatSend }},
		...
		{Method: "GET", Path: "/v1/projects/{id}/bugs/{bug}/attachments/{name}", Auth: true,
			Summary:      "Serve one bug attachment",
			ClientMethod: "BugAttachment",
			handler:      func(s *Server) http.HandlerFunc { return s.handleBugAttachment }},
		...
		{Method: "POST", Path: "/v1/projects/{id}/artifacts/{kind}/lint", Auth: true,
			Request: service.ArtifactLintRequest{}, Response: service.ArtifactLintResult{},
			Summary: "Preflight candidate artifact grammar without writing a proposal or running graph/semantic checks", ClientMethod: "ArtifactLint",
			handler: func(s *Server) http.HandlerFunc { return s.handleArtifactLint }},
		...
		{Method: "GET", Path: "/v1/projects/{id}/next", Auth: true,
			Response: listOf{Items: []service.NextStep{}}, Summary: "The project's suggested next steps",
			ClientMethod: "ProjectNext", handler: func(s *Server) http.HandlerFunc { return s.handleProjectNext }},
		{Method: "GET", Path: "/v1/projects/{id}/next/{ref}", Auth: true,
			Response: service.Journey{}, Summary: "One bug's or task's journey: its ladder position and the door that is next",
			ClientMethod: "NextFor", handler: func(s *Server) http.HandlerFunc { return s.handleNextFor }},
