...
// appendArtifactCriticPolicy keeps every route that reviews a materialized
// document on the same artifact-specific contract. The ordinary council loop
// and the bounded final review used to construct their prompts separately;
// the latter consequently lost the plan obligation audit exactly when it was
// checking the candidate offered to the human.
func appendArtifactCriticPolicy(prompt string, turn *Turn, script *Script) string {
	if turn == nil || turn.Persona != PersonaCritic || kindOfContract(turn.Contract, script) != "plan" {
		return prompt
	}
