...
func TestStructureCheckFlagsSubNumberedHeadings(t *testing.T) {
	...
	findings := structureFindings(nil, cur, "markdown_sections:REQ", nil)
...
func TestOneDeliverablesPerTaskIsNotADuplicate(t *testing.T) {
	...
	if findings := structureFindings(nil, []agent.Section{milestone}, "markdown_sections:M", nil); len(findings) != 0 {
		t.Fatalf("a well-formed milestone was flagged: %v", findings)
	}
	dup := agent.Section{ID: "M-01", Title: "Core", Body: "### T-001 — Scaffold\n\n**Deliverables:**\n- a\n\n**Deliverables:**\n- b\n"}
	findings := structureFindings(nil, []agent.Section{dup}, "markdown_sections:M", nil)
