...
func runSectioned(ctx context.Context, p Params, base *artifact.Document, ask string) (*Result, error) {
	...
	var ids, adds []string
	completed := 0
	// A shallow copy aliases each milestone's Children slice. Replacing one
	// task would then mutate base too, erasing the very before-image the final
	// composition review must compare against.
	proposedDoc, cloneErr := artifact.Parse(artifact.RenderBody(base), kind)
	if cloneErr != nil {
		return nil, fmt.Errorf("clone sectioned base: %w", cloneErr)
	}
	proposed := *proposedDoc
	proposed.Front = base.Front
