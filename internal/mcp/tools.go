...
func (s *Server) roster(a args) (map[string]interface{}, error) {
	...
	// Seat candidates ride on the entries the engine already resolved
	// (service.RankCandidates is the one ranking rule); this surface passes
	// them through and never recomputes them — the MCP server is a client.
...
func (s *Server) runGet(id string) (map[string]interface{}, error) {
	...
	view := pick(run, "id", "project_id", "stage", "mode", "task_id", "status", "verdict",
		"failure", "redo_note", "resolution", "pending_kind", "pending_data", "next", "budget", "warning", "findings", "stage_request")
