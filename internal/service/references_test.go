...
func TestStructuredJSONReferencesLoadAsExecutableContracts(t *testing.T) {
  ...
  "cases":[{"id":"empty","expected":{"findings":[]}}]
	...
	for _, want := range []string{"Executable reference contracts", "```ducklab-reference-contracts", `"observe_gate": {`, `"required": [`, `"optional": []`, `"additional_properties": false`, `"findings"`, contractPath, "sha256:"} {
