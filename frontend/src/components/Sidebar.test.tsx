...
const zones = [
  ...
  { label: "Work", testid: "nav-work", home: { name: "board" as const }, members: ["board" as const, "cycle" as const, "flock" as const] },
...
const config = [
  ...
  { label: "Flock", route: { name: "flock" as const } },
...
describe("desktop sidebar rail", () => {
  it("keeps primary navigation, settings rooms, and footer status reachable", () => {
    render(<Sidebar route={{ name: "now" }} zones={zones} configMembers={["settings", "flock", "skills", "projects", "ducklings"]} subnav={{ Config: config }} projects={[]} projectId="" onProject={() => {}} client={null} waitingCount={2} connection="open" />);
    ...
    expect(screen.queryByTestId("subnav-flock")).not.toBeInTheDocument();
  ...
  it("keeps every routed view reachable, with settings rooms behind its internal menu", () => {
    const routedViews = ["now", "board", "cycle", "runs", "reports", "review", "release", "bench", "settings", "ducklings", "flock", "skills", "projects"];
    const sidebarTree = [...zones.flatMap((zone) => [zone.home.name, ...zone.members]), "reports", "review", "release", "bench", "settings"];
    const settingsMenu = ["ducklings", "flock", "skills", "projects"];
  ...
  it("attaches every configured route to a sidebar door", () => {
    render(<Sidebar route={{ name: "settings" }} zones={zones} configMembers={["settings", "flock", "skills", "projects", "ducklings"]} subnav={{ Config: config }} projects={[]} projectId="" onProject={() => {}} client={null} waitingCount={0} connection="open" />);
