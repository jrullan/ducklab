...
type RankedCandidate = { id: string; why: string };

// Keep this fallback byte-for-byte compatible with service.RankCandidates. It
// is only used for older roster views that predate the candidates field.
const rankCandidates = (role: string, cards: Scorecard[]): RankedCandidate[] => {
  if (role === "triager" || role === "scribe" || role === "human") return [];
  const ranked = cards.flatMap((s) => {
    const m = s.measured;
    if (!m?.runs || s.locality === "local") return [];
    const cost = m.avg_cost_usd ?? 0;
    const wall = m.avg_wallclock_s ?? 0;
    const pass = m.pass_rate ?? 0;
    const benchScores = Object.values(s.bench ?? {}).map((b) => b.score).filter((v): v is number => v !== undefined);
    const hasBench = benchScores.length > 0;
    const bench = hasBench ? Math.max(...benchScores) : 0;
    let primary = pass;
    let secondary = cost;
    let why = `pass rate ${Math.round(pass * 100)}% over ${m.runs} runs · $${cost.toFixed(2)}/run`;
    if ((role === "implementer" || role === "architect") && hasBench) {
      primary = bench;
      why = `bench ${Math.round(bench * 100)} · $${cost.toFixed(2)}/run`;
    }
    if (role === "advisor") {
      primary = cost;
      secondary = wall;
      why = `$${cost.toFixed(2)}/run · ${Math.round(wall)}s avg`;
    }
    return [{ id: s.id, why, primary, secondary, hasBench }];
  });
  ranked.sort((a, b) => {
    if (a.hasBench !== b.hasBench && (role === "implementer" || role === "architect")) return a.hasBench ? -1 : 1;
    if (a.primary !== b.primary) return role === "advisor" ? a.primary - b.primary : b.primary - a.primary;
    if (a.secondary !== b.secondary) return a.secondary - b.secondary;
    return a.id.localeCompare(b.id);
  });
  return ranked.slice(0, 3).map(({ id, why }) => ({ id, why }));
};
