...
describe("remote diagnostics", () => {
  ...
  it("offers and persists the per-project publication policy", async () => {
    ...
    const client = {
      ...
      projectGet: vi.fn().mockResolvedValue({ config: { remote: { name: "origin", on_accept: "push" }, github: { pr_base: "main" } } }),
      projectUpdate,
    ...
    const policy = await screen.findByTestId("remote-on_accept");
    expect(policy).toHaveValue("push");
    expect(Array.from((policy as HTMLSelectElement).options).map((option) => option.value)).toEqual(["nothing", "push", "pr"]);
    fireEvent.change(policy, { target: { value: "pr" } });
    fireEvent.click(screen.getByTestId("save-remote-git-settings"));
    expect(projectUpdate).toHaveBeenCalledWith("p", expect.objectContaining({ "remote.on_accept": "pr" }), "settings_remote_git");
  });
