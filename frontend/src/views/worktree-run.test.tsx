...
describe("RunView worktree surface", () => {
  ...
  it("lists a paused rebase conflict and its only lawful options", async () => {
    ...
    expect(card).toHaveTextContent("rebase is stopped with conflict markers");
    expect(card).toHaveTextContent("git rebase --continue");
    ...
    expect(screen.getByTestId("decision-consequence")).toHaveTextContent("only after the rebase has been completed");
  });
});
